  ...
  try {
    ...
    for (const uid of notifySet) {
      await Notification.create({
        ...
        title: 'New Comment', message: `${req.user.name} commented on a task`,
        link: `/projects/${task.project}`, data: { taskId }
      });
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    req.io.to(`project:${task.project}`).emit('comment:deleted', { taskId: comment.task, commentId: req.params.id });
